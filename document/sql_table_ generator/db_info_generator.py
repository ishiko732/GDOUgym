...
class DbInfoGenerator:
    ...
    def gen_table_column_info(self, table_info_row, file_path,
                              col_names_skip=None):
        # 导出字段信息表到文档
        ...
        # 写入excel文件
        self.save_column_info_to_excel(table_name, table_comment, column_info,
                                       file_path, col_names_skip)

    def save_column_info_to_excel(self, table_name, table_comment, column_info,
                                  file_path, col_names_skip):
        # 写入表信息到excel文件
    ...
    def set_file_format(self, file_path):
        ...
        for i in range(1, max_row + 1):
            ...
            if col1_value and not col2_value:
                # 合并表名
                sheet.merge_cells(start_row=i, start_column=1, end_row=i,
                                  end_column=4)
                # 加粗字体
                ...
                border = Border(left=side_style, right=side_style,
...
if __name__ == '__main__':
    excel_path = "/Users/liuyuanfeng/Desktop/gym/word/tableinfo.xlsx"

    host = "1.15.187.187"
    port = 3306
    user = "GSM"
    password = "GSM1935gdou"
    db_name = "GSM"
    charset = 'utf8'
    ...
    dbInfoGenerator = DbInfoGenerator(host, port, user, password, db_name,
                                      charset)
    dbInfoGenerator.create_file(excel_path)
    # dbInfoGenerator.gen_db_table_info(db_name, excel_path)
    dbInfoGenerator.gen_db_table_info_skip_col(db_name, excel_path,
                                               col_names_to_skip)
